...
public class ResolutionManager : MonoBehaviour
{
	...
	public void SetFullscreen(bool b)
	{
		...
		SetResolution(
			b? currFullscreenRes : currWindowedRes, b);
	}


	public List<string> GetFullscreenResolutionsStringList ()
	{
		return BuildListIntoStringList (FullscreenResolutions);
	}

	public List<string> GetWindowedResolutionsStringList ()
	{
		return BuildListIntoStringList (WindowedResolutions);
	}


	public List<string> BuildListIntoStringList (List<Vector2> list) 
	{
		List<string> resolutionsStringList = new List<string> ();

		for (int i = 0; i < list.Count; i++) 
		{
			string newResolutionString = list [i].x.ToString () + " x " + list [i].y.ToString ();
			resolutionsStringList.Add (newResolutionString);
		}

		return resolutionsStringList;
	}


	public int GetCurrentFullscreenResIndex ()
	{
		return currFullscreenRes;
	}
